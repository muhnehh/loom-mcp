...
function initParser(lang: any): Parser | null {
  const p = new Parser();
  try {
    p.setLanguage(lang);
  } catch {
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('function ') || line.startsWith('export function ')) {
        ...
        if (match) funcs.push({ name: match[1], kind: 'function', params: match[2] || '', returnType: match[3] || 'void', lineStart: i, lineEnd: i });
      } else if (line.startsWith('interface ') || line.startsWith('type ')) {
        const match = line.match(/(?:interface|type)\s+(\w+)/);
        if (match) funcs.push({ name: match[1], kind: 'type', lineStart: i, lineEnd: i });
      } else if (line.startsWith('class ')) {
        const match = line.match(/class\s+(\w+)/);
        if (match) funcs.push({ name: match[1], kind: 'class', lineStart: i, lineEnd: i });
      }
    }
